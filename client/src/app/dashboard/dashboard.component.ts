	...
	ngOnInit(): void {
		...
		forkJoin({
			...
			next: (data) => {
				...
				const weekStartForLogs = startOfWeek(new Date(), { weekStartsOn: 1 });
				const weekDateStrs = new Set(
					Array.from({ length: 7 }, (_, i) => format(addDays(weekStartForLogs, i), 'yyyy-MM-dd'))
				);
